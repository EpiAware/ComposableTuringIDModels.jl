...
uses, so the importation rate can equally be a fixed constant, an unknown
constant, or a time-varying process such as a [`RandomWalk`](@ref) — in every
case the prior is on the log scale, like every other unknown positive quantity
here.
"""
