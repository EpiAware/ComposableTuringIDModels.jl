...
An [`IDProblem`](@ref) holds a model and its data together, so [`data_requirements`](@ref) reads the observation count off the data rather than being told it.
[`forecast`](@ref) extends the observations by the horizon and derives the rest.
