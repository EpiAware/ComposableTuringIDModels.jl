...
CDF-scaling conditions on — see the consistency note below).
...
[`TriangleObs`](@ref) observation model.

## Consistency with CDF-scaling
...
`μ_t · F[(now − t) + 1]`, exactly the CDF-scaled expected observed-so-far. So the
observed row-sums of the triangle are the marginal that the EpiNow2-style
CDF-scaling nowcast conditions on; the triangle additionally models how that
total is split across delays.

## Fields

  - `counts`: the `N[t, d+1]` count matrix (reference day × delay). Unobserved
    cells are ignored by [`TriangleObs`](@ref); they may hold `missing`, `0`, or
    any placeholder.
...
rt = define_y_t(TriangleObs(PoissonError(), [0.5, 0.3, 0.2]), N, fill(20.0, 4))
rt.observed
```
"
...
@doc raw"
A **2D reporting-triangle** observation model (epinowcast-style nowcasting).

`TriangleObs` consumes the expected eventual totals `Y_t = μ_t` (per reference
...
triangle's per-cell means and the CDF-scaling nowcast share one delay kernel.

## Constructors

  - `TriangleObs(error_model, pmf)` — from a delay PMF (non-negative, sums to 1);
    `Dmax = length(pmf) - 1`.
  - `TriangleObs(error_model, distribution; D, Δd)` — discretise a continuous
...
Pass `y_t = missing` to **simulate**: `TriangleObs` builds a fully observed
...
obs = TriangleObs(PoissonError(), truncated(Normal(2.0, 1.0), 0.0, Inf))
# Simulate a triangle for 15 reference days of expected total 50.
sim = as_turing_model(obs, missing, fill(50.0, 15))()
sim.observed
```
"
struct TriangleObs{E <: AbstractObservationErrorModel, T <: AbstractVector{<:Real}} <:
    ...
    function TriangleObs(error_model::E,
            pmf::T) where {
...
function TriangleObs(error_model::E, distribution::C; D = nothing,
        Δd = 1.0) where {
        E <: AbstractObservationErrorModel, C <: ContinuousDistribution}
    pmf = _discretised_pmf(distribution; Δd = Δd, D = D)
    return TriangleObs(error_model, pmf)
end

# The maximum reporting delay carried by the model (delays `0 … Dmax`).
_triangle_Dmax(o::TriangleObs) = length(o.pmf) - 1

@doc raw"
Build the [`ReportingTriangle`](@ref) data a [`TriangleObs`](@ref) scores.

Dispatched on [`TriangleObs`](@ref) as the triangle method of the shared
...
# Arguments
  ...
  - `obs_model`: the [`TriangleObs`](@ref) model (fixes `Dmax`).
...
```
"
function define_y_t(obs_model::TriangleObs, y_t::ReportingTriangle, Y_t)
...
function define_y_t(obs_model::TriangleObs, y_t::Missing, Y_t)
    n = length(Y_t)
    Dmax = _triangle_Dmax(obs_model)
...
function define_y_t(obs_model::TriangleObs, y_t::AbstractMatrix, Y_t; now::Int = size(
        y_t, 1))
    n = size(y_t, 1)
    Dmax = _triangle_Dmax(obs_model)
...
function define_y_t(obs_model::TriangleObs, reports, Y_t; now::Int,
...
@model function as_turing_model(obs_model::TriangleObs, y_t, Y_t)
