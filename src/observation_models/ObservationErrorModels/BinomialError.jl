...
# step, so the two natural shapes — one entry per observation, or one per
# expected value — put the same trials on the same time points. Trials that do
# not reach back to the first scored step are rejected rather than quietly
# shifted.
