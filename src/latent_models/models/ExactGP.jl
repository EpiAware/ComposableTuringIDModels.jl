...
@model function _exact_gp_model(
    ...
    # A fixed absolute nugget is swamped once σ is large (the factorisation
    # then fails on a numerically-indefinite matrix, ending the chain); a
    # jitter-scaled nugget swamps the covariance when σ is small. The absolute
    # floor `jitter * eps()` is far below σ² for any representable σ and only
    # keeps the factorisation defined at σ = 0 exactly.
