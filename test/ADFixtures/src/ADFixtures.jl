...
function _models()
    ...
    modifiers = IDModel(
        Renewal(
            gen_int, SusceptibleDepletion(2_000.0),
    ...
    binom_obs = Ascertainment(
        BinomialError(), HierarchicalNormal();
    ...
    patch_pooled = IDModel(
        Renewal(;
            generation_time = gen_int, rt = rt_process,
            initialisation = Normal(log(50.0), 0.2)
        ),
        PoissonError()
    )
    ...
    patch_mixg = IDModel(
        Renewal(;
            ...
            initialisation = Normal(log(50.0), 0.2),
            mixing = Gravity(
                patch_pop, patch_dist; α = HalfNormal(1.0),
