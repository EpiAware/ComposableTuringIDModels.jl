...
# A report is a known DynamicPPL `@model` false positive when it is an
# undefined-local-variable report (`UndefVarErrorReport`) arising inside a
# `@model`-generated method — i.e. a method whose instance takes the DynamicPPL
# evaluator arguments `(::DynamicPPL.Model, ::DynamicPPL.AbstractVarInfo, ...)`.
# Those undefined locals are the `~`-assigned variables the tilde macro hides
# from JET's static analysis; they are well-defined at run time.
function is_model_tilde_false_positive(report)
...
real_reports = filter(!is_model_tilde_false_positive, reports)

if isempty(real_reports)
    n = length(reports) - length(real_reports)
    println("JET: no real errors ($(n) DynamicPPL @model tilde false positives filtered)")
    exit(0)
else
