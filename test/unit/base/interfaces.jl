...
@testitem "role slots reject wrong-role components at construction" begin
    ...
    # IDProblem takes an IDModel, so a bare component cannot stand in for one.
    @test_throws MethodError IDProblem(obs, fill(5, 10))
...
@testitem "reusable interface checkers confirm role conformance" begin
    ...
    @test implements_prior_interface(HilbertSpaceGP(); n = 25)
    @test implements_prior_interface(HilbertSpaceGP(; kernel = Matern52Kernel()); n = 25)
    @test implements_prior_interface(ExactGP(); n = 25)
